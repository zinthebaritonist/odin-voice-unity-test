...
### 前提条件
- Unity 6 (6000.0.62f1) - **このプロジェクトは Unity 6 で開発・テスト済み**
- マイク（内蔵でOK）
- Git
...
##### ステップ4: .envファイルに設定
1. プロジェクトルートに`.env`ファイルが既に存在
2. **チームから提供されたルームトークン（JWT）**を設定:
```
ODIN_ACCESS_TOKEN=チームから提供されたJWTトークンをここに入力
ODIN_DEFAULT_ROOM=TestRoom
```

**🔑 チーム向け:**
- 全員が同じアクセスキーから生成されたルームトークンを使用してください
- ルームトークンはチームメンバーに個別に配布してください
- セキュリティのため、トークンの共有は安全な方法で行ってください
